...
$scheduleQueries = [
    "SELECT ttSchedDate AS scheduleDate, 'Tetanus' AS scheduleType, CONCAT(p.patientFName, ' ', p.patientMname, ' ', p.patientLname) AS patientName 
     FROM tetanus_sched ts 
     LEFT JOIN patient p ON ts.patientID = p.patientID 
     WHERE p.patientID = '$patientID'",
     
    "SELECT calSchedDate AS scheduleDate, 'Calcium' AS scheduleType, CONCAT(p.patientFName, ' ', p.patientMname, ' ', p.patientLname) AS patientName 
     FROM calcium_sched cs 
     LEFT JOIN patient p ON cs.patientID = p.patientID 
     WHERE p.patientID = '$patientID'",
     
    "SELECT dwSchedDate AS scheduleDate, 'Deworming' AS scheduleType, CONCAT(p.patientFName, ' ', p.patientMname, ' ', p.patientLname) AS patientName 
     FROM deworming_sched ds 
     LEFT JOIN patient p ON ds.patientID = p.patientID 
     WHERE p.patientID = '$patientID'",
     
    "SELECT ironSchedDate AS scheduleDate, 'Micronutrient' AS scheduleType, CONCAT(p.patientFName, ' ', p.patientMname, ' ', p.patientLname) AS patientName 
     FROM micronutrient_sched ms 
     LEFT JOIN patient p ON ms.patientID = p.patientID 
     WHERE p.patientID = '$patientID'",
     
    "SELECT pnSchedCheckupDate AS scheduleDate, 'Pre-natal' AS scheduleType, CONCAT(p.patientFName, ' ', p.patientMname, ' ', p.patientLname) AS patientName 
     FROM `pre-natal_sched` ps 
...
foreach ($scheduleQueries as $query) {
    $result = mysqli_query($conn, $query);
    while ($scheduleRow = mysqli_fetch_assoc($result)) {
        $schedules[] = [
            'date' => $scheduleRow['scheduleDate'],
            'type' => $scheduleRow['scheduleType'],
            'patient' => $scheduleRow['patientName'],
            'patientID' => $patientID  
        ];
    }
}
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <h5 class="text-center" id="scheduleModalLabel">Patient Schedule Details</h5>
                <p><strong>Patient Name:</strong> <span id="modalPatientName"></span></p>
                <p><strong>Schedule Type:</strong> <span id="modalScheduleType"></span></p>
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col">
                                ...
                                <input type="number" class="form-control" id="patientSerialNumber" name="patientSerialNumber" value="<?php echo $row['patientSerialNumber'];?>" placeholder="Patient's family serial no." required readonly>
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-4">
                                <label>First Name</label>
                                <input type="text" name="patientFirstName" class="form-control" value="<?php echo $row['patientFname'];?>" placeholder="Patient's first name" required readonly>
                            </div>
                            <div class="form-group col-md-4">
                                <label>Middle Name</label>
                                <input type="text" name="patientMiddleName" class="form-control" value="<?php echo $row['patientMname'];?>" placeholder="Patient's middle name" required readonly>
                            </div>
                            <div class="form-group col-md-4">
                                <label>Last Name</label>
                                <input type="text" name="patientLastName" class="form-control" value="<?php echo $row['patientLname'];?>" placeholder="Patient's last name" required readonly>
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-3">
                                <label>Birthday</label>
                                <input type="date" name="patientBirthday" id="birthday" value="<?php echo $row['patientBirthday'];?>" class="form-control" required readonly>
                            </div>
                            <div class="form-group col-md-3">
                                <label>Age</label>
                                <input type="number" name="patientAge" id="age" class="form-control" value="<?php echo $row['patientAge'];?>" placeholder="Patient's age" readonly required>
                            </div>
                            <div class="form-group col-md-3">
                                <label>Weight (kg)</label>
                                <input type="number" name="patientWeight" id="weight" class="form-control" value="<?php echo $row['patientWeight'];?>" placeholder="Patient's weight in kilograms" step="0.01" required readonly>
                            </div>
                            <div class="form-group col-md-3">
                                <label>Height (m)</label>
                                <input type="number" name="patientHeight" id="height" class="form-control" value="<?php echo $row['patientHeight'];?>" placeholder="Patient's height in meters" step="0.01" required readonly> 
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-4">
                                <label>BMI Result</label>
                                <input type="text" name="patientBMIResult" id="bmi" class="form-control" value="<?php echo $row['patientBMI'];?>" placeholder="Patient's BMI result" readonly required>
                            </div>
                            <div class="form-group col-md-4">
                                <label>BMI Category</label>
                                <input type="text" name="patientBMICategory" id="bmiCategory" class="form-control" value="<?php echo $row['patientBMICategory'];?>" placeholder="Patient's BMI category" readonly required>
                            </div>
                            <div class="form-group col-md-4">
                        ...
                        <div class="row">
                            <div class="form-group col-md-4">
                                ...
                                <input type="text" name="patientBMICategory" id="bmiCategory" class="form-control" value="<?php echo $row['barangayName'];?>" placeholder="Patient's BMI category" readonly required>
                            </div>
                            <div class="form-group col-md-4">
                                <label>Contact No.</label>
                                <input type="number" name="patientContactNumber" class="form-control" maxlength="11" value="<?php echo $row['patientContactNumber'];?>" placeholder="Patient's contact number" required readonly>
                            </div>
                            <div class="form-group col-md-4">
                                <label>Emergency Contact</label>
                                <input type="number" name="patientEmergencyContact" maxlength="11" class="form-control" value="<?php echo $row['patientEmergencyContact'];?>" placeholder="Patient's emergency contact number" required readonly>
                            </div>
                        </div>
                ...
                <div id="collapsePage6" class="collapse">
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            <div class="form-group col-md-4">
                                <label>Type</label>
                                <select disabled class="form-control" id="place_of_delivery_type" name="edit_place_of_delivery_type">
                                    <option value="" <?php if (empty($row['poPlaceType'])) echo 'selected'; ?> disabled>Select Delivery Place Type</option>
                            ...
                            <div class="form-group col-md-2">
                                ...
                                <select disabled class="form-control" id="birth_attendant" name="edit_birth_attendant">
                                    ...
                                    <option value="H" <?php if ($row['poBirthAttendant'] === 'H') echo 'selected'; ?>>Hilot/TBA</option>
                                    <option value="0" <?php if ($row['poBirthAttendant'] === '0') echo 'selected'; ?>>Others</option>
                                </select>
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventClick: function(info) {
            document.getElementById('modalPatientName').innerText = info.event.extendedProps.patientName;
            document.getElementById('modalScheduleType').innerText = info.event.extendedProps.scheduleType;
            document.getElementById('modalScheduleDate').innerText = info.event.extendedProps.scheduleDate;
